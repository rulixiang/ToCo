...
from model.losses import get_masked_ptc_loss, get_seg_loss, CTCLoss, DenseEnergyLoss, get_energy_loss
from model.model_seg import network
...
from utils.camutils import cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi
...
def train(args=None):
    ...
    ncrops = 10
    CTC_loss = CTCLoss(out_dim=1024, ncrops=ncrops,).cuda()
    ...
    for n_iter in range(args.max_iters):
        ...
        if n_iter <= 8000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.2 * ptc_loss + 0.5 * ctc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.2 * ptc_loss + 0.5 * ctc_loss + 0.1 * seg_loss + 0.05 * reg_loss
